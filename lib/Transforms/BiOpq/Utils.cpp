...
void fixStack(Function *f) {
  ...
  BasicBlock *bbEntry = (BasicBlock*) f->begin();
  ...
  do {
    ...
    for (Function::iterator bbIt = f->begin(); bbIt != f->end(); ++bbIt) {
      ...
      for (BasicBlock::iterator it = bb->begin(); it != bb->end(); ++it) {
        ...
        if (!(isa<AllocaInst>(inst) && inst->getParent() == bbEntry) &&
            (valueEscapes(inst) || it->isUsedOutsideOfBlock(bb))) {
